...
\newenvironment{myquotation}
               {\list{}{\listparindent=0pt%whatever you need
                        ...
                        \leftmargin=14pt%  whatever you need
                        \rightmargin=14pt%whatever you need
...
%\begin{myquotation}
%this is on purpose. if not, the code in the next page will be
%scattered into 2 pages
\vspace{-2mm}
\begin{quotation}
...
terminate the application --- doing so is considered preferable
...
A Google search for ``SWTException:Invalid thread access''  returns over 11,800 hits,
...
and discussion forum returns over 2700 bug reports and 350 distinct discussion threads, respectively. 
...
revision d7fa4814218d. On line 74, \CodeIn{LayerView}'s constructor
...
Such an approach is desirable for accesses from non-UI threads, but it is
not necessary for all accesses.
...
Furthermore, this approach is dangerous for accesses from the UI thread. Asynchronous message passing offers
...
\vspace*{-15pt}
\Caption{{\label{fig:report} Our tool reports
...
``\CodeIn{{...}}" above, are omitted for brevity.
}} %\vspace{-5mm}
\end{figure}
...
conservative nature, or may report multiple warnings that actually correspond
to the same error. To address such limitations, we devised a set
...
We implemented an invalid-thread-access-error detection tool that supports
...
Eclipse is the most widely-used IDE for Java. Android is the \#1 platform for
...
and thus the programmer effort.
...
invalid thread access errors as an elegant call graph reachability problem,
...
%  LocalWords:  multithreaded SWT plugin MacOS myquotation 0pt 10pt GL10
%  LocalWords:  SWTException LayerView LayerController mRenderer mGLThread
%  LocalWords:  LayerRenderer setRenderer GLThread onSurfaceChanged gl RTA
%  LocalWords:  glViewport mView setViewportSize IntSize Fennec GUIs init
%  LocalWords:  GLSurfaceView guardedRun checkThread CFA
