...
call graph 
%for partial programs such as an Eclipse plugin, or
...
with an explicit object creation call: \CodeIn{new Button()}.
...
(\CodeIn{SwingUti-\\lities.invokeLater} and \CodeIn{SwingUtilities.invokeAndWait}) to execute code in the UI thread.
...
all public methods from an \CodeIn{Activity} class as call graph entry nodes.
...
%, since invoking them on any thread
%would not cause an invalid thread access error.
