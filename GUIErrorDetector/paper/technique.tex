...
and safe methods include two SWT helper methods \CodeIn{Display.asyncExec}  and
...
and also stores all visited nodes to avoid infinite loop (line 10).
...
by a non-UI thread, it computes full method call chain from the entry node to
the UI-accessing node as the error report.
The method \CodeIn{createErrorReport} on line 20 does that. 
As an example, the method call chain for the error in in Figure~\ref{fig:androiderror}
is shown in Figure~\ref{fig:report}.

Our algorithm in Figure~\ref{fig:detectalgorithm} uses BFS to search
for potential error-revealing paths, since BFS permits to produce
small error reports (i.e., shorter method call chains). However,
...
we empirically compared different graph search strategies, and demonstrate that
...
\STATE $\mathit{queue}$ $\leftarrow$ an empty queue\\
\COMMENT{keep all visited nodes to avoid infinite loop}
\STATE $\mathit{visited}$ $\leftarrow$ $\emptyset$
\STATE $\mathit{queue}$.enqueue(getSuccNodes($\mathit{cg}$, $\mathit{threadStart}$))
\WHILE{$\mathit{queue}$.isNotEmpty()}
\STATE $\mathit{node}$ $\leftarrow$ $\mathit{queue}$.dequeue()
...
\STATE continue
\ELSE
\STATE $\mathit{queue}$.enqueue($\mathit{succNode}$)
\ENDIF 
\ENDFOR
\ENDWHILE
\ENDFOR
...
%this is wrong for k-CFA, k-CFA maintains several sets per expression, which k > 0
%should cite Ben Livshit's paper on "Reflection analysis for Java"
%Donot overclaim "say propose a new technique", instead, say "refine the technique"
%also need to cite Yannis's paper on solving k-CFA paradox

%\textbf{need to describe why need to traverse from the UI entry, not just from the thread entry}
...
existing call graph construction algorithms such as RTA and k-CFA
fail to build a sufficiently complete call graph.
...
the second step is to use a standard call graph construction algorithm
to construct the graph on the transformed program. The reflection call
represents a method invocation that relies Java reflection to create
objects, such as the \CodeIn{findViewById} method in Android applications.
...
in an analyzed program. When it sees a reflection call, the algorithm analyzes the
program to determine a set of possible classes that might be created.
After that, the algorithm creates a new statement which non-deterministically
...
The algorithm in Figure~\ref{fig:cgalgorithm} is parameterized in three places:
line 2, line 3, and line 8. We instantiated it for Android applications as
follows. On line 2, the predicate $isReflectionCall$ returns true
...
file in an Android application to extract the class declaration
 correspondinig to the \CodeIn{id} value. If the \CodeIn{id}
value is dynamically generated, the method will conservatively
return all subclasses of the declared type. On line 8, any existing
call graph construction algorithm can be used to build the call graph
on top of the transformed program.

For the example in Figure~\ref{fig:sampleandroid}, the algorithm
first parses the XML configuration file to determine that the \CodeIn{button\_id}
value is mapped to a \CodeIn{Button} instance declaration. Then it
replaces the reflection call: \CodeIn{findViewById(R.id.button\_id)} 
with an explicit object creation call: \CodeIn{new Button(null)}.
...
\item \textbf{Safe methods: } two helper methods (\CodeIn{View.post}
