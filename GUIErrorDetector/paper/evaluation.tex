...
have known invalid thread access errors, all of which have been fixed later.
We ran our tool on the reported buggy versions to check whether it
can correctly identify the existing errors. 
We selected the other five subjects by first searching for the
framework keywords (e.g., ``Java Swing'' or ``Java SWT'')
in the above source repositories, and then chosing subjects based on the following
...
We ran our tool on the latest stable release of each project to check whether
...
annotations for it. 
%Two subjects, MyTracks and Fennec, employ a customized pattern
%to interact with the GUI framework. For example, in MyTracks, all non-UI
%threads are initialized via using the library method \CodeIn{android.os.}\discretionary{}{}{}\CodeIn{handler.handleCallback}.
...
%message passing. 
We also added \filternum user-defined filters for two subjects
(MyTracks and Fennec), since they use a customized pattern
to interact with the GUI framework.
...
by either searching the bug repository to check it with the bug-fixing revision history,
...
%After checking against existing patches, we found
Our tool correctly identified the \oldbugs known errors, as compared to
their patches.
We submitted all \newbugs new errors
to the subject developers. As of April 2012, 1 error in S3dropbox has been confirmed,
we have reproduced 3 errors, and 3 errors in VirgoFTP, FileBunker and ArecaBackup
...
Another error in SGTPuzzler is only reported when adding \annotationnum native method
...
E and U, while  an unsound strategy might report an infeasible
path between the nodes.  Exhaustive search is sound because it does not
miss any possible paths.  However, it is impractical.  Compared to
exhaustive search, BFS or DFS is unsound because they visit
every node, but do not traverse \textit{every} path to visit that node.
...
physical memory (1GB is allocated for JVM), running Windows 7.
