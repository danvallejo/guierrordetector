...
computes Sudoku solutions efficiently using mutlithreaded execution. 
...
user's default mail client.% (after a user clicks the "eMail Me" button).
...
lines 6 and 23 in Figure~\ref{fig:detectalgorithm}, and changing
...
\noindent This variant returns the shortest path from \textit{any} \CodeIn{Thread.start()} node
to \textit{any} UI-accessing node, rather than the shortest path from \textit{a given}
\CodeIn{Thread.start()} node to \textit{any} UI-accessing node as the algorithm in
...
in an error path from a \CodeIn{Thread.start()} node, that node will not be visited again
even when the standard BFS traverses from another \CodeIn{Thread.start()} node, thus
missing potential errors.

The second variant uses DFS for graph traversal by changing
the data structure of $\mathit{worklist}$ on line 7 in Figure~\ref{fig:detectalgorithm} to
a stack, and changing queue operations (\CodeIn{dequeue} and \CodeIn{enqueueAll}
