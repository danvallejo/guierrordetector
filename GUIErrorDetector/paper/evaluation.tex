...
\hline
\hline
 Total & 199161 / 199724 &  188 / 251 & 6 / 6 & 149441 / 153054& 137 / 137& 7 / 7 & 577821 / 652756& 22 / 22 & 11 / 12 & 6542\\
\hline
\end{tabular}
\end{center}
\vspace{-15pt}
...
results of a straightforward approach, called ``Requiring Wrappers'' (Section~\ref{sec:straightforward}),
...
introducing imprecision. Although the error-revealing path exists in
the less precise graphs, it presents the error detection algorithm finding it.
Figure~\ref{fig:ex} illustrates this limitation.

\begin{figure}[t]
  \centering
  \includegraphics[scale=0.33]{cgexample}
  \vspace*{-5.0ex}\caption {{\label{fig:ex} (a) shows an example code, in which
\CodeIn{safeMtd} is a Safe UI method, and \CodeIn{uiAcc} is a UI-Accessing method.
(b) shows a less precise call graph built by RTA or 0-CFA, and (c) shows a more
precise call graph built by 1-CFA. The node for \CodeIn{Thread.run()}
method is omitted for brevity. Using the less precise call graph, the error
detection algorithm will report an invalid method call chain: \CodeIn{entry()}
$\rightarrow$ \CodeIn{f()} $\rightarrow$ \CodeIn{start()} $\rightarrow$ \CodeIn{uiAcc()},
but miss the actual error path: \CodeIn{entry()} $\rightarrow$ \CodeIn{g1()} $\rightarrow$
\CodeIn{g2()} $\rightarrow$ \CodeIn{start()} $\rightarrow$ \CodeIn{uiAcc()}. The
