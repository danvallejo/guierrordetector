...
\CodeIn{Thread.start()} calls (line 7). Each \CodeIn{Thread.start()} call
...
%we augument k-CFA with value set
...
\vspace{-5mm}
\begin{algorithmic}[1]
\FOR{each $statement$ in $P$}
\IF{$statement$ is a reflection call}
...
they fail to conclude that variable \CodeIn{button} declared at line 7
point to a \CodeIn{Button} object due to its limitation in deal with the
reflection call \CodeIn{findViewById}. Thus, the resulting
...
the \CodeIn{findViewById} method), the algorithm analyzes the
...
replaces the \CodeIn{findViewById(R.id.button\_id)} method call 
with an explicit object creation instruction \CodeIn{new Button}.
...
and \CodeIn{Display.syncExec}) to execute code (a)synchronizedly on the UI thread.
...
%An eclipse plugin is an open program. 
Unlike a SWT desktop application, an Eclipse plugin does not
...
%due to the complexity of the eclipse
%architecture,
...
%by the UI thread. XXX This rule, though quite , works quite well in practice.
...
%it is a Swing listener class. If so, we extract all its overridden callback methods as the call graph entry points.
...
(\CodeIn{SwingUti-\\lities.invokeLater} and \CodeIn{SwingUtilities.invokeAndWait}) to pass messages to the UI thread.
...
Thus, we treat every methods calling \CodeIn{ViewRoot.checkThread} as a UI-accessing node
in the call graph.
